...
public sealed class ActivitiesController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ActionResult<PagedResult<ActivityDto>>> Get([FromQuery] int page = 1, [FromQuery] int pageSize = 20, CancellationToken cancellationToken = default)
    {
        return Ok(await _activityService.GetPagedAsync(page, Math.Clamp(pageSize, 1, 100), cancellationToken));
    }
